...
static NSTimeInterval const kDrawerAnimationDuration = 0.10;  //0.25;
...
- (IBAction)didTapProfileView:(id)sender
{
    if ([self.delegate respondsToSelector:@selector(drawerController:stateClosed:)]) {
        [self.delegate drawerController:self stateClosed:KYDrawerControllerDrawerStateClosed];
    }
}
...
- (void)goToTargetViewController:(UIViewController *)targetVC {
    
    [self setDrawerState:KYDrawerControllerDrawerStateClosed animated:YES];
    [self performSelector:@selector(pushTargetViewController:) withObject:targetVC afterDelay:0.3];
}

- (void)pushTargetViewController:(UIViewController *)targetVC {
    
    UINavigationController* localNavigationController = [[UINavigationController alloc] initWithRootViewController:targetVC];
    [self setMainViewController:localNavigationController];
